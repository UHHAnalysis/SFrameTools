...
int SubJetFlavor(TopJet topjet, int whichsub, float matchcone)
{
    ...
    Particle subjet=subjets_top[whichsub];
	...
	std::vector<double> matched_genparticle_pts;
        for(unsigned int i=0; i<genparticles->size(); ++i) {
            ...
            if(subjet.deltaR(genp)<matchcone){
                matched_genparticle_ids.push_back(genp.pdgId());
		matched_genparticle_pts.push_back(genp.pt());
	    }
        }
	...
	for(unsigned int i=0; i<matched_genparticle_ids.size(); ++i) {
	  if(abs(matched_genparticle_ids[i])==3 && matched_genparticle_pts[i]>ptcomp) { 
	    hardestflav=matched_genparticle_ids[i];
	    ptcomp=matched_genparticle_pts[i];	    
	  }
	  if(abs(matched_genparticle_ids[i])==2 && matched_genparticle_pts[i]>ptcomp) { 
	    hardestflav=matched_genparticle_ids[i];
	    ptcomp=matched_genparticle_pts[i];	    
	  }
	  if(abs(matched_genparticle_ids[i])==1 && matched_genparticle_pts[i]>ptcomp) { 
	    hardestflav=matched_genparticle_ids[i];
	    ptcomp=matched_genparticle_pts[i];	    
	  }
	  if(abs(matched_genparticle_ids[i])==21 && matched_genparticle_pts[i]>ptcomp) { 
	    hardestflav=matched_genparticle_ids[i];
	    ptcomp=matched_genparticle_pts[i];	    
	  }
...
int subJetBTagOne(TopJet topjet, E_BtagType type, TString mode, TString filename, int whichsub){
  ...
  for(unsigned int i=0; i < btagsub_combinedSecondaryVertex_top.size(); ++i){
    ...
    BtagScale* bscale = new BtagScale(type);
    CtagScale* cscale = new CtagScale(type);
    LtagScale* lscale = new LtagScale(type);

    if(abs(flav)==1||abs(flav)==2||abs(flav)==3||abs(flav)==21){
      SF=lscale->value(subpt,subeta);
    }
    else if(abs(flav)==4){
      SF=cscale->value(subpt,subeta);
    }
    else if(abs(flav)==5){
      SF=bscale->value(subpt,subeta);
    }
    ...
    if((syst=="bcup"||syst=="bcupdelta")&&(abs(flav)==5)){
      SF = bscale->value_plus(subpt,subeta);
      if(isdelta[i]){
	SF=SF+(SF-refcsv);
      }
    }
    if((syst=="bcdown"||syst=="bcdowndelta")&&(abs(flav)==5)){
      SF = bscale->value_minus(subpt,subeta);
      if(isdelta[i]){
	SF=SF+(SF-refcsv);
      }
    }
    if((syst=="bcup"||syst=="bcupdelta")&&(abs(flav)==4)){
      SF = cscale->value_plus(subpt,subeta);
      if(isdelta[i]){
	SF=SF+(SF-refcsv);
      }
    }
    if((syst=="bcdown"||syst=="bcdowndelta")&&(abs(flav)==4)){
      SF = cscale->value_minus(subpt,subeta);
      if(isdelta[i]){
	SF=SF+(SF-refcsv);
      }
    }
    ...
    if((syst=="lightup"||syst=="lightupdelta")&&(abs(flav)==1||abs(flav)==2||abs(flav)==3||abs(flav)==21)){
      SF = lscale->value_plus(subpt, subeta);
      if(isdelta[i]){
	SF=SF+(SF-refcsv);
      }
    }
    ...
    if((syst=="lightdown"||syst=="lightdowndelta")&&(abs(flav)==1||abs(flav)==2||abs(flav)==3||abs(flav)==21)){
      SF = lscale->value_minus(subpt, subeta);
      if(isdelta[i]){
	SF=SF+(SF-refcsv);
      }
    }
